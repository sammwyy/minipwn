//! Worker server: the daemon side that exposes shell/system endpoints over HTTP.
//!
//! Started by `minipwn worker`. [`routes`] wires the Axum router with bearer
//! auth, [`handlers`] implements each endpoint, and [`state`] holds the shared
//! config and live shell sessions.
